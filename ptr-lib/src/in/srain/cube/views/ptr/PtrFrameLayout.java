...
public class PtrFrameLayout extends ViewGroup {
    ...
    private int mDurationToCloseHeader = 1000;//fragment切换时动画执行时间
    ...
    private int mLoadingMinTime = 500;  //手动下拉刷新时动画执行时间
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mDisableWhenHorizontalMove && !mPreventForHorizontal && (Math.abs(offsetX) > mPagingTouchSlop / 2 && Math.abs(offsetX) > Math.abs(offsetY))) {
